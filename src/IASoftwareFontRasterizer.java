...
final class IASoftwareFontRasterizer extends AbstractFontRasterizer
{
    ...
    IASoftwareFontRasterizer(SoftwareToolkit toolkit, BitmapFont font, ImageSprite[] sprites,
		  int[] is, int[] is_0_) {
	super(toolkit, font);
	...
	for (int i = 0; i < sprites.length; i++) {
	    ImageSprite class207 = sprites[i];
	    if (((ImageSprite) class207).alphaIndex != null)
		alphaIndices[i] = ((ImageSprite) class207).alphaIndex;
	    else {
		byte[] is_1_ = ((ImageSprite) class207).colorIndex;
		byte[] is_2_ = alphaIndices[i] = new byte[is_1_.length];
		for (int i_3_ = 0; i_3_ < is_1_.length; i_3_++)
		    is_2_[i_3_] = (byte) (is_1_[i_3_] == 0 ? 0 : -1);
	    }
	    heightOffsets[i] = ((ImageSprite) class207).heightOffset;
	    widthOffsets[i] = ((ImageSprite) class207).widthOffset;
	}
    }

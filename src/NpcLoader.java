...
final class NpcLoader
{
    ...
    private Cache definitionCache = new Cache(64);
	...
	synchronized (definitionCache) {
	    ...
	    definitionCache.removeAll(i + 6080);
	}
	synchronized (((NpcLoader) this).aClass60_3590) {
    ...
    final NpcDefinition getNpcDefinition(int id) {
	anInt3586++;
	NpcDefinition definition;
	synchronized (definitionCache) {
	    definition = (NpcDefinition) definitionCache.method583((long) id, -104);
	}
	if (definition != null)
	    return definition;
	byte[] payload;
	...
	if (payload != null)
	    definition.initialize(111, new ByteBuffer(payload));
	definition.method799(-117);
	synchronized (definitionCache) {
	    definitionCache.method582(definition, (long) id, (byte) -102);
	}
	return definition;
    }

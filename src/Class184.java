...
final class Class184
{
	...
	if (bool) {
	    Class167.parseWorldListUpdate((byte) -113, (((ByteBuffer) Class299_Sub1_Sub2.lobbylistBuffer).payload));
	    Class65.lobbyWorlds = new LobbyWorld[Class225.amountWorlds];
	    int i_2_ = 0;
	    for (int i_3_ = Class318_Sub1_Sub2.minimumWorldId; ((i_3_ ^ 0xffffffff) >= (Class239_Sub29.maximumWorldId ^ 0xffffffff)); i_3_++) {
		LobbyWorld info = Class239_Sub5.getLobbyWorld(i_3_);
		if (info != null) {
		    Class65.lobbyWorlds[i_2_++] = info;
                }
	    }
	    Class130.lobbyUpdateSent = false;

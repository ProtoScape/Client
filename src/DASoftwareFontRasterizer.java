...
final class DASoftwareFontRasterizer extends AbstractFontRasterizer
{
    ...
    DASoftwareFontRasterizer(SoftwareToolkit toolkit, BitmapFont font, ImageSprite[] class207s,
		  int[] is, int[] is_103_) {
	super(toolkit, font);
	...
	for (int i = 0; i < class207s.length; i++) {
	    colorIndices[i] = ((ImageSprite) class207s[i]).colorIndex;
	    heightOffsets[i] = ((ImageSprite) class207s[i]).heightOffset;
	    widthOffsets[i] = ((ImageSprite) class207s[i]).widthOffset;
	}
    }
}

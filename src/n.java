...
final class n extends AbstractFontRasterizer implements ModelToolkit
{
    ...
    n(SafeToolkit var_oa, ya var_ya, BitmapFont class143, ImageSprite[] class207s,
	...
	for (int i = 0; i < class207s.length; i++) {
	    is[i] = ((ImageSprite) class207s[i]).colorIndex;
	    is_5_[i] = ((ImageSprite) class207s[i]).indexWidth;
	    is_6_[i] = ((ImageSprite) class207s[i]).indexHeight;
	    is_7_[i] = ((ImageSprite) class207s[i]).widthOffset;
	    is_8_[i] = ((ImageSprite) class207s[i]).heightOffset;
	}
	S(var_oa, var_ya, is, ((ImageSprite) class207s[0]).colors, is_5_,
	  is_6_, is_7_, is_8_);
    }

...
final class h extends AbstractFontRasterizer implements ModelToolkit
{
    ...
    h(SafeToolkit var_oa, ya var_ya, BitmapFont class143, ImageSprite[] class207s,
	...
	for (int i = 0; i < class207s.length; i++) {
	    ...
	    is_0_[i] = ((ImageSprite) class207s[i]).indexWidth;
	    is_1_[i] = ((ImageSprite) class207s[i]).indexHeight;
	    is_2_[i] = ((ImageSprite) class207s[i]).widthOffset;
	    is_3_[i] = ((ImageSprite) class207s[i]).heightOffset;
	}

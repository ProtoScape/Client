...
final class ArchiveFetcher
{
    ...
    static GamePacket aClass351_4223 = new GamePacket(4, 0);
    static int anInt4224;
    static GamePacket aClass351_4225 = new GamePacket(43, -1);
	...
	if (encipheredDigest.length != 128)
	    throw new RuntimeException();
	byte[] digest = Class348_Sub1_Sub2.getWhirlpoolDigest((((ByteBuffer) buffer).payload), 5, 6 + (entries * 72));
	...
	try {
	    ...
	    int crc = Client.skipUpdate ? 0 : parentTableBuffer.getDword();
	    int version = Client.skipUpdate ? 0 : parentTableBuffer.getDword();
	    byte[] is = new byte[64];
            if(!Client.skipUpdate)

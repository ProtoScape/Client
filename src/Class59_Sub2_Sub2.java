...
final class Class59_Sub2_Sub2 extends Class59_Sub2
{
    ...
    static ServerPacket aClass114_8683 = new ServerPacket(100, -1);
    static int[] anIntArray8684;
    static int anInt8685;
    
    static final void rebuildWorldList(ByteBuffer buffer) {
	anInt8675++;
	int amountLocations = buffer.getSmartB();
	MouseMovementListener.worldInfo = new WorldInfo[amountLocations];
	for (int i_1_ = 0; i_1_ < amountLocations; i_1_++) {
	    MouseMovementListener.worldInfo[i_1_] = new WorldInfo();
	    ((WorldInfo) MouseMovementListener.worldInfo[i_1_]).country = buffer.getSmartB();
	    ((WorldInfo) MouseMovementListener.worldInfo[i_1_]).region = buffer.getJStr2(-13487);
	}
	Class318_Sub1_Sub2.minimumWorldId = buffer.getSmartB();
	Class239_Sub29.maximumWorldId = buffer.getSmartB();
	Class225.amountWorlds = buffer.getSmartB();
	OutputStream_Sub1.lobbyWorlds = new LobbyWorld[1 + (Class239_Sub29.maximumWorldId - Class318_Sub1_Sub2.minimumWorldId)];
	for (int i = 0; i < Class225.amountWorlds; i++) {
	    int offset = buffer.getSmartB();
	    LobbyWorld class110_sub1 = (OutputStream_Sub1.lobbyWorlds[offset] = new LobbyWorld());
	    ((AbstractWorld) class110_sub1).infoId = buffer.getUByte();
	    ((AbstractWorld) class110_sub1).settingFlags = buffer.getDword();
	    ((LobbyWorld) class110_sub1).worldId = Class318_Sub1_Sub2.minimumWorldId + offset;

...
final class Class348_Sub42_Sub6 extends SubNode
{
    ...
    static final void parseWorldListStats(byte i, ByteBuffer class348_sub49) {
	anInt9539++;
	for (int i_0_ = 0; (Class225.amountWorlds ^ 0xffffffff) < (i_0_ ^ 0xffffffff); i_0_++) {
	    int worldId = class348_sub49.getSmartB();
	    int amountPlayers = class348_sub49.getShort();
	    if ((amountPlayers ^ 0xffffffff) == -65536)
		amountPlayers = -1;
	    if (OutputStream_Sub1.lobbyWorlds[worldId] != null)
		((AbstractWorld) OutputStream_Sub1.lobbyWorlds[worldId]).amountPlayers = amountPlayers;
	}
	int i_3_ = -75 / ((53 - i) / 48);
    }
	...
	if (i == 0) {
		...
		for (int i_4_ = 0; i_4_ < strings.length; i_4_++) {
			...
			ClientApplet.addConsoleMessage(("Pausing for " + i_5_
					      + " seconds..."));
